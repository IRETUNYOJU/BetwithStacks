...
(define-public (place-bet (wager-id uint) (chosen-outcome uint) (bet-amount uint))
  (let
    ...
    (asserts! (< stacks-block-height (get end-block wager)) ERR-WAGER-EXPIRED)
...
(define-public (close-wager (wager-id uint))
  (let
    ...
    (asserts! (get active wager) ERR-WAGER-CLOSED)
    (asserts! (>= stacks-block-height (get end-block wager)) ERR-WAGER-NOT-READY-TO-CLOSE)
    (map-set wagers
      { wager-id: wager-id }
...
(define-public (cancel-wager (wager-id uint))
  (let
    ...
    (asserts! (get active wager) ERR-WAGER-CLOSED)
    (asserts! (< stacks-block-height (get end-block wager)) ERR-WAGER-NOT-CANCELABLE)
